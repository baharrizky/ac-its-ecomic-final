...
import { loadTeacherDataBundle } from "./services/teacherDataService";
...
export default function App(){
 ...
 const [teacherDataError,setTeacherDataError]=useState(null);
 ...
 const refreshTeacherData=async()=>{
   ...
   const bundle=await loadTeacherDataBundle(teacherUid);
   if(!bundle.ok){
     setTeacherDataError(bundle);
     setRegisteredStudents([]);
     setTeacherClasses([]);
     setTeacherData({models:[],attempts:[],reflections:[],attendance:[],examResults:[],events:[]});
     // Never show a false zero when the local session and Firebase Auth differ.
     if(bundle.code==="AUTH_SESSION_MISMATCH") {
   ...
   setTeacherClasses(bundle.classes||[]);
   setRegisteredStudents(bundle.students||[]);
   setTeacherData({models:bundle.models||[],attempts:bundle.attempts||[],reflections:bundle.reflections||[],attendance:bundle.attendance||[],examResults:bundle.examResults||[],events:bundle.events||[]});
 ...
 useEffect(()=>{
   ...
   (async()=>{
     ...
     const [refs,att,events,examResults]=await Promise.all([listReflections({uid:session.uid}),listAttendance({uid:session.uid}),listLearningEvents({uid:session.uid}),listExamResults({uid:session.uid})]);
     if(alive){
       setStudentReflections(refs);setStudentAttendance(att);setStudentExamResults(examResults);
       let clearedAt=0;
 ...
 useEffect(()=>{
   let alive=true;let unsubscribe=()=>{};
   (async()=>{const cloud=await loadCloudComics(session);if(!alive)return;if(cloud!==null){setState(s=>({...s,comics:cloud}));unsubscribe=await subscribeCloudComics(session, items=>{if(alive)setState(s=>({...s,comics:items}));});}})();
   return()=>{alive=false;unsubscribe?.();};
 },[session?.role]);
 useEffect(()=>{
   let alive=true;let unsubscribe=()=>{};
 ...
 const visibleStudentQuestions=useMemo(()=>{const qs=Array.isArray(state.questions)?state.questions:[];return qs.filter(q=>q.status!=="Draft" && (q.assessmentType||"practice")==="practice" && (!q.educationLevel||q.educationLevel===session?.educationLevel) && (!q.grade||q.grade===session?.grade));},[state.questions,session?.educationLevel,session?.grade]);
 const leaderboard=useMemo(()=> (Array.isArray(registeredStudents)?registeredStudents:[]).filter(s=>!session?.school||!s.school||s.school===session.school).filter(s=>!session?.grade||s.grade===session.grade).filter(s=>!session?.rombel||s.rombel===session.rombel),[registeredStudents,session?.school,session?.grade,session?.rombel]);
 const teacherVisibleStudents=useMemo(()=>{if(session?.role!=="teacher")return [];const classes=Array.isArray(teacherClasses)?teacherClasses:[];const students=Array.isArray(registeredStudents)?registeredStudents:[];const ids=new Set(classes.map(c=>c.id));return students.filter(s=>s.classTeacherUid===session.uid || (s.classId&&ids.has(s.classId)));},[registeredStudents,teacherClasses,session?.uid,session?.role]);
 const visibleStudentIds=useMemo(()=>new Set(teacherVisibleStudents.map(s=>s.uid)),[teacherVisibleStudents]);
 const teacherAttempts=useMemo(()=> (Array.isArray(teacherData?.attempts)?teacherData.attempts:[]).filter(a=>visibleStudentIds.has(a.uid)),[teacherData.attempts,visibleStudentIds]);
 const teacherReflections=useMemo(()=> (Array.isArray(teacherData?.reflections)?teacherData.reflections:[]).filter(r=>visibleStudentIds.has(r.uid)),[teacherData.reflections,visibleStudentIds]);
 const teacherAttendance=useMemo(()=> (Array.isArray(teacherData?.attendance)?teacherData.attendance:[]).filter(r=>visibleStudentIds.has(r.uid)),[teacherData.attendance,visibleStudentIds]);
 const teacherExamResults=useMemo(()=> (Array.isArray(teacherData?.examResults)?teacherData.examResults:[]).filter(r=>visibleStudentIds.has(r.uid)),[teacherData.examResults,visibleStudentIds]);
 const teacherLearningEvents=useMemo(()=> (Array.isArray(teacherData?.events)?teacherData.events:[]).filter(e=>visibleStudentIds.has(e.uid)),[teacherData.events,visibleStudentIds]);
 ...
 const handleCreate=async form=>{if(!teacherClasses.length){setState(x=>({...x,screen:"teacher-access"}));return;}const target=teacherClasses.find(x=>x.school===session?.school&&x.educationLevel===form.educationLevel&&x.grade===form.grade)||teacherClasses.find(x=>x.educationLevel===form.educationLevel&&x.grade===form.grade)||teacherClasses[0];const assignedClassIds=teacherClasses.filter(x=>(target?.school?x.school===target.school:true)&&x.educationLevel===target?.educationLevel&&x.grade===target?.grade&&x.active!==false).map(x=>x.id);const c=createComic({...form,school:target?.school||session?.school||"",educationLevel:target?.educationLevel||form.educationLevel,grade:target?.grade||form.grade,rombel:"",classId:"",className:"",classTeacherUid:session?.uid||"",ownerTeacherUid:session?.uid||"",assignedClassIds,createdBy:session?.uid||"current-teacher"});const ok=await saveCloudComic(c);if(!ok){console.error("COMIC_CLOUD_SAVE_FAILED",c.id);return;}setState(s=>({...s,comics:[c,...s.comics.filter(x=>x.id!==c.id)],selectedComicId:c.id,screen:"comic-editor"}));};
 const handleSaveComic=async u=>{const school=u.school||session?.school||"";const assignedClassIds=teacherClasses.filter(x=>(!school||x.school===school)&&x.educationLevel===u.educationLevel&&x.grade===u.grade&&x.active!==false).map(x=>x.id);const updated={...u,school,rombel:"",classId:"",className:"",ownerTeacherUid:u.ownerTeacherUid||session?.uid||"",assignedClassIds,createdBy:u.createdBy||session?.uid||"current-teacher",updatedAt:new Date().toISOString().slice(0,10)};const ok=await saveCloudComic(updated);if(!ok){console.error("COMIC_CLOUD_SAVE_FAILED",updated.id);return false;}setState(s=>({...s,comics:updateComic(s.comics,updated)}));return true;};
 const handleSaveQuestion=async q=>{if(!teacherClasses.length){setState(x=>({...x,screen:"teacher-access"}));return;}const level=q.educationLevel||"SMA";const grade=q.grade||"X";const school=q.school||session?.school||"";const assigned=teacherClasses.filter(c=>(!school||c.school===school)&&c.educationLevel===level&&c.grade===grade&&c.active!==false).map(c=>c.id);const payload={...q,school,educationLevel:level,grade,rombel:"",classId:"",className:"",ownerTeacherUid:session?.uid||"",assignedClassIds:assigned,createdBy:session?.uid||"current-teacher",updatedAt:new Date().toISOString()};setState(s=>({...s,questions:[...s.questions.filter(x=>x.id!==q.id),payload]}));const ok=await saveCloudQuestion(payload);if(!ok)console.error("QUESTION_CLOUD_SAVE_FAILED",payload.id);return ok;};
 ...
 const handleAnswer=async(q,aIndex,modeName="practice",durationSeconds=0,hintsUsed=0)=>{
 ...
 const handleReaderProgress=async({comic,episodeIndex,panelIndex,durationSeconds=0})=>{if(!session?.uid||!comic)return;const key=`${comic.id}:${episodeIndex}:${panelIndex}`;await recordLearningEvent({uid:session.uid,name:session.name,role:"student",teacherUid:session.classTeacherUid||null,classId:session.classId||null,type:"comic_panel_view",comicId:comic.id,episodeIndex,panelIndex,durationSeconds:Number(durationSeconds)||0,createdAt:new Date().toISOString()});if(state.studentModel.completedPanels?.[key]){setState(s=>({...s,currentReaderContext:{comicId:comic.id,episodeIndex,panelIndex}}));return;}const currentConceptIds=Array.isArray(comic.episodes?.[episodeIndex]?.panels?.[panelIndex]?.conceptIds)?comic.episodes[episodeIndex].panels[panelIndex].conceptIds:[];const exposed={...(state.studentModel.concepts||{})};currentConceptIds.forEach(id=>{const old=exposed[id]||{mastery:0.1,confidence:0.25,attempts:0,correct:0};exposed[id]={...old,exposureCount:Number(old.exposureCount||0)+1};});const next=touchActivity({...state.studentModel,concepts:exposed,completedPanels:{...(state.studentModel.completedPanels||{}),[key]:1}},2);const allDone=(comic.episodes||[]).every(ep=>(ep.panels||[]).every((_,pi)=>next.completedPanels?.[`${comic.id}:${comic.episodes.indexOf(ep)}:${pi}`]));if(allDone&&!next.completedComics.includes(comic.id))next.completedComics=[...(next.completedComics||[]),comic.id];setState(s=>({...s,studentModel:next,currentReaderContext:{comicId:comic.id,episodeIndex,panelIndex}}));await saveStudentModel(session.uid,next);};
 const handleReflection=async r=>{const item={...r,uid:session.uid,teacherUid:session.classTeacherUid||null,classId:session.classId||null,createdAt:new Date().toISOString(),date:new Date().toISOString().slice(0,10)};await saveReflection(item);setStudentReflections(await listReflections({uid:session.uid}));if(session?.uid)await recordLearningEvent({uid:session.uid,teacherUid:session.classTeacherUid||null,classId:session.classId||null,type:"reflection_submitted",payload:{date:item.date}});};
 ...
 const handleExamComplete=async(result,eligible,answers)=>{let next=touchActivity(state.studentModel, result.score>=80?30:10);for(let i=0;i<eligible.length;i++){const q=eligible[i];if(answers[i]==null)continue;const d=diagnoseAnswer(q,answers[i]);next=updateMastery(next,q.conceptId,d);await recordAttempt({uid:session.uid,name:session.name,teacherUid:session.classTeacherUid||null,classId:session.classId||null,questionId:q.id,conceptId:q.conceptId,selectedIndex:answers[i],correct:d.correct,score:d.correct?100:0,mode:"exam",educationLevel:session.educationLevel,grade:session.grade,school:session.school,examId:`exam-${Date.now()}`});}next.examScore=result.score;setState(s=>({...s,studentModel:next}));await saveStudentModel(session.uid,next);await saveExamResult({...result,uid:session.uid,name:session.name,school:session.school,educationLevel:session.educationLevel,grade:session.grade,rombel:session.rombel,teacherUid:session.classTeacherUid||null,classId:session.classId||null});setStudentExamResults(prev=>[{...result,uid:session.uid,grade:session.grade,educationLevel:session.educationLevel},...prev]);await refreshTeacherData();};
 const handlePracticeHint=async({question,hintIndex,conceptId,conceptName})=>{
   const profile=state.studentModel?.concepts?.[conceptId]||{};
   const latestMis=Array.isArray(state.studentModel?.misconceptions)?state.studentModel.misconceptions.find(m=>m.conceptId===conceptId&&!m.resolved):null;
   let ai=null;
   try{
     ai=await getAIHint({question,hintIndex,conceptId,conceptName,studentMastery:profile.mastery||0,misconceptionTag:latestMis?.tag||null,context:{comicId:question?.comicId||"",panelId:question?.panelId||"",educationLevel:session.educationLevel,grade:session.grade}});
...
function TeacherGrades({students,session,questions,attempts=[],events=[],examResults=[],onRefresh,teacherClasses=[]}){
 ...
 const avg=(key)=>visible.length?Math.round(visible.reduce((sum,s)=>sum+Number(s[key]||0),0)/visible.length):0; const attemptCount=new Map();attempts.forEach(a=>attemptCount.set(a.uid,(attemptCount.get(a.uid)||0)+1)); const practiceMap=new Map();attempts.filter(a=>a.mode==="practice").forEach(a=>{const x=practiceMap.get(a.uid)||[];x.push(Number(a.score||0));practiceMap.set(a.uid,x)}); const eventCount=new Map();events.forEach(e=>eventCount.set(e.uid,(eventCount.get(e.uid)||0)+1)); const examMap=new Map(examResults.map(r=>[r.uid,r])); const avgPractice=visible.length?Math.round(visible.reduce((sum,s)=>{const x=practiceMap.get(s.uid)||[];return sum+(x.length?x.reduce((a,b)=>a+b,0)/x.length:0)},0)/visible.length):0;
 return <div><div className="page-kicker">Student Assessment</div><h1 className="page-title">Nilai Siswa</h1><p className="page-desc">Filter sekolah, kelas, dan rombel. Siswa mengikuti kelas yang dikelola Guru.</p><div className="card" style={{marginBottom:18}}><div className="section-head"><div><h2>Kontrol Kelas</h2><span>Pilih ruang kelas yang sedang dilihat.</span></div><button className="btn" onClick={onRefresh}>↻ Perbarui</button></div><div className="register-grid"><div><label className="label">Sekolah</label><select value={school} onChange={e=>{setSchool(e.target.value);setGrade("");setRombel("")}}><option value="">Semua sekolah</option>{schools.map(v=><option key={v}>{v}</option>)}</select></div><div><label className="label">Kelas</label><select value={grade} onChange={e=>{setGrade(e.target.value);setRombel("")}}><option value="">Semua kelas</option>{grades.map(v=><option key={v}>{v}</option>)}</select></div><div><label className="label">Rombel</label><select value={rombel} onChange={e=>setRombel(e.target.value)}><option value="">Semua rombel</option>{rombels.map(v=><option key={v}>{v}</option>)}</select></div></div></div><div className="stats-row"><div className="ac-stat"><div className="stat-icon purple">▣</div><div><span>Rata-rata Mastery</span><strong>{avg("mastery")}%</strong></div></div><div className="ac-stat"><div className="stat-icon blue">✓</div><div><span>Siswa</span><strong>{visible.length}</strong></div></div><div className="ac-stat"><div className="stat-icon green">★</div><div><span>Rata-rata Latihan</span><strong>{avgPractice}</strong></div></div><div className="ac-stat"><div className="stat-icon orange">✎</div><div><span>Bank Soal</span><strong>{questions.length}</strong></div></div></div><div className="card"><h2>Rekap Nilai</h2>{visible.length?<table className="table"><thead><tr><th>Siswa</th><th>Sekolah</th><th>Kelas</th><th>Nilai Latihan</th><th>Nilai Ujian</th><th>Mastery</th><th>Aktivitas</th><th>XP</th></tr></thead><tbody>{visible.map(s=><tr key={s.uid||s.email}><td><strong>{s.name}</strong></td><td>{s.school||"-"}</td><td>{s.grade||"-"} {s.rombel||""}</td><td>{practiceMap.get(s.uid)?.length ? Math.round(practiceMap.get(s.uid).reduce((a,b)=>a+b,0)/practiceMap.get(s.uid).length) : "-"}</td><td>{examMap.get(s.uid)?.score ?? s.exam ?? 0}</td><td>{s.mastery||0}%</td><td>{eventCount.get(s.uid)||0}</td><td>{s.xp||0}</td></tr>)}</tbody></table>:<div className="empty-state"><strong>{teacherClasses.length?"Belum ada siswa pada kelas ini.":"Belum ada kelas."}</strong><span>Daftar akan terisi otomatis setelah siswa terhubung ke kelas.</span></div>}</div></div>
}
