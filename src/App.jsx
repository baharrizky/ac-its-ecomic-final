...
export default function App(){
 ...
 const refreshTeacherData=async()=>{
   ...
   const classes=teacherUid ? await listClassesForTeacher(teacherUid) : [];
   const rawStudents=await getRegisteredStudents(teacherUid, classes);
   ...
   if (session?.role === "teacher") setTeacherClasses(classes);
   return students;
 };
 ...
 const handleCreate=form=>{if(!teacherClasses.length){setState(x=>({...x,screen:"teacher-access"}));return;}const target=teacherClasses.find(x=>x.id===form.classId)||teacherClasses.find(x=>x.school===form.school&&x.grade===form.grade&&x.rombel===form.rombel)||teacherClasses[0];const c=createComic({...form,school:target?.school||form.school||session?.school||"",educationLevel:target?.educationLevel||form.educationLevel,grade:target?.grade||form.grade,rombel:target?.rombel||form.rombel||form.className,classId:target?.id||form.classId||"",className:target?.rombel||form.className||"",classTeacherUid:session?.uid||"",ownerTeacherUid:session?.uid||"",assignedClassIds:form.assignedClassIds?.length?form.assignedClassIds:[target?.id].filter(Boolean),createdBy:session?.uid||"current-teacher"});setState(s=>({...s,comics:[c,...s.comics],selectedComicId:c.id,screen:"comic-editor"}));saveCloudComic(c);};
 ...
 const renderTeacher=()=>{switch(state.screen){case"comic-management":return <ComicManagement comics={state.comics} teacherClasses={teacherClasses} session={session} navigate={navigate} onCreate={handleCreate} onEdit={id=>navigate("comic-editor",id)}/>;case"comic-editor":return <ComicEditorPage comic={selectedComic} session={session} onBack={()=>navigate("comic-management")} onSave={handleSaveComic}/>;case"comic-preview":return <ComicReaderPage comic={selectedComic} studentModel={emptyModel(availableConcepts)} questions={state.questions} navigate={navigate} session={session}/>;case"question-bank":return <QuestionBankPage questions={state.questions} onSave={handleSaveQuestion} onDelete={handleDeleteQuestion}/>;case"analytics":return <AnalyticsPage students={teacherVisibleStudents} models={teacherData.models} attempts={teacherAttempts} teacherClasses={teacherClasses} onRefresh={refreshTeacherData}/>;case"teacher-grades":return <TeacherGrades students={teacherVisibleStudents} session={session} questions={state.questions} teacherClasses={teacherClasses} onRefresh={refreshTeacherData}/>;case"teacher-question-progress":return <TeacherQuestionProgress questions={state.questions} attempts={teacherAttempts}/>;case"teacher-ranking":return <TeacherRanking students={teacherVisibleStudents}/>;case"teacher-exam-times":return <TeacherExamTimes examResults={teacherExamResults} students={teacherVisibleStudents}/>;case"teacher-knowledge":return <TeacherKnowledge/>;case"teacher-reflections":return <TeacherReflections reflections={teacherReflections} students={teacherVisibleStudents}/>;case"teacher-access":return <TeacherAccess session={session} onClassesChange={setTeacherClasses}/>;case"teacher-attendance":return <TeacherAttendance records={teacherAttendance} students={teacherVisibleStudents} teacherClasses={teacherClasses}/>;case"teacher-activity":return <TeacherActivityPage students={teacherVisibleStudents} events={teacherLearningEvents} onRefresh={refreshTeacherData}/>;case"teacher-migration":return <TeacherMigrationPage session={session}/>;case"teacher-profile":return <TeacherProfile session={session}/>;default:return <TeacherDashboard state={state} navigate={navigate} students={teacherVisibleStudents} models={teacherData.models} attempts={teacherAttempts} events={teacherLearningEvents} session={session} onRefresh={refreshTeacherData} onAIRecommend={getTeacherRecommendation}/>} };
...
function TeacherGrades({students,session,questions,onRefresh,teacherClasses=[]}){
