...
export default function App(){
 ...
 const handleCreate=async form=>{setOperationError("");if(!teacherClasses.length){setState(x=>({...x,screen:"teacher-access"}));throw new Error("Buat atau buka kelas Guru terlebih dahulu.");}const c=createComic({...form,school:session?.school||"",rombel:form.rombel||session?.rombel||"",classTeacherUid:session?.uid||"",ownerTeacherUid:session?.uid||"",assignedClassIds:teacherClasses.map(x=>x.id),createdBy:session?.uid||"current-teacher"});await saveCloudComic(c);setState(s=>({...s,comics:[c,...s.comics],selectedComicId:c.id,screen:"comic-editor"}));return c;};
 const handleSaveComic=async u=>{setOperationError("");const updated={...u,school:u.school||session?.school||"",ownerTeacherUid:u.ownerTeacherUid||session?.uid||"",assignedClassIds:Array.isArray(u.assignedClassIds)&&u.assignedClassIds.length?u.assignedClassIds:teacherClasses.map(x=>x.id),createdBy:u.createdBy||session?.uid||"current-teacher",updatedAt:new Date().toISOString().slice(0,10)};await saveCloudComic(updated);setState(s=>({...s,comics:updateComic(s.comics,updated)}));return true;};
 const handleSaveQuestion=async q=>{setOperationError("");if(!teacherClasses.length){setState(x=>({...x,screen:"teacher-access"}));throw new Error("Buat atau buka kelas Guru terlebih dahulu.");}const payload={...q,ownerTeacherUid:session?.uid||"",assignedClassIds:teacherClasses.map(x=>x.id),createdBy:session?.uid||"current-teacher",updatedAt:new Date().toISOString()};await saveCloudQuestion(payload);setState(s=>({...s,questions:[...s.questions.filter(x=>x.id!==q.id),payload]}));return true;};
 const handleDeleteQuestion=async id=>{setOperationError("");await deleteCloudQuestion(id);setState(s=>({...s,questions:s.questions.filter(q=>q.id!==id)}));};
 ...
 return <><FlexibleStyles/><div className="ac-app"><FlexibleSidebar open={drawerOpen} mode={mode} screen={state.screen} items={mode==="admin"?adminItems:mode==="teacher"?teacherItems:studentItems} onNavigate={(screen)=>navigate(screen)} onLogout={onLogout} session={session} onClose={()=>setDrawerOpen(false)}/>{drawerOpen&&<button aria-label="Tutup menu" className="ecomic-drawer-backdrop" onClick={()=>setDrawerOpen(false)}/>}<div className="ac-main"><FlexibleTopbar mode={mode} session={session} onMenu={()=>setDrawerOpen(v=>!v)}/><main className="ac-content">{operationError&&<div className="upload-note error" style={{marginBottom:14}}>{operationError}</div>}{mode==="admin"?renderAdmin():mode==="teacher"?renderTeacher():renderStudent()}</main></div></div></>;
}
