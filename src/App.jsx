...
export default function App(){
 ...
 const handleCreate=async form=>{if(!teacherClasses.length){setState(x=>({...x,screen:"teacher-access"}));return;}const target=teacherClasses.find(x=>x.id===form.classId)||teacherClasses.find(x=>x.school===form.school&&x.grade===form.grade&&x.rombel===form.rombel)||teacherClasses[0];const c=createComic({...form,school:target?.school||form.school||session?.school||"",educationLevel:target?.educationLevel||form.educationLevel,grade:target?.grade||form.grade,rombel:target?.rombel||form.rombel||form.className,classId:target?.id||form.classId||"",className:target?.rombel||form.className||"",classTeacherUid:session?.uid||"",ownerTeacherUid:session?.uid||"",assignedClassIds:form.assignedClassIds?.length?form.assignedClassIds:[target?.id].filter(Boolean),createdBy:session?.uid||"current-teacher"});const ok=await saveCloudComic(c);if(!ok){console.error("COMIC_CLOUD_SAVE_FAILED",c.id);return;}setState(s=>({...s,comics:[c,...s.comics.filter(x=>x.id!==c.id)],selectedComicId:c.id,screen:"comic-editor"}));};
 const handleSaveComic=async u=>{const updated={...u,school:u.school||session?.school||"",ownerTeacherUid:u.ownerTeacherUid||session?.uid||"",assignedClassIds:Array.isArray(u.assignedClassIds)&&u.assignedClassIds.length?u.assignedClassIds:teacherClasses.map(x=>x.id),createdBy:u.createdBy||session?.uid||"current-teacher",updatedAt:new Date().toISOString().slice(0,10)};const ok=await saveCloudComic(updated);if(!ok){console.error("COMIC_CLOUD_SAVE_FAILED",updated.id);return false;}setState(s=>({...s,comics:updateComic(s.comics,updated)}));return true;};
 const handleSaveQuestion=async q=>{if(!teacherClasses.length){setState(x=>({...x,screen:"teacher-access"}));return;}const assigned=Array.isArray(q.assignedClassIds)&&q.assignedClassIds.length?q.assignedClassIds:[q.classId||teacherClasses[0]?.id].filter(Boolean);const target=teacherClasses.find(c=>c.id===assigned[0])||teacherClasses[0];const payload={...q,school:q.school||target?.school||session?.school||"",educationLevel:q.educationLevel||target?.educationLevel||"SMA",grade:q.grade||target?.grade||"X",rombel:q.rombel||target?.rombel||target?.name||"",classId:target?.id||q.classId||"",className:target?.rombel||q.className||"",ownerTeacherUid:session?.uid||"",assignedClassIds:assigned,createdBy:session?.uid||"current-teacher",updatedAt:new Date().toISOString()};setState(s=>({...s,questions:[...s.questions.filter(x=>x.id!==q.id),payload]}));const ok=await saveCloudQuestion(payload);if(!ok)console.error("QUESTION_CLOUD_SAVE_FAILED",payload.id);return ok;};
