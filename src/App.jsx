...
import { getTutorReply, correctAnswerWithAI, generateNextPracticeQuestion, getTeacherRecommendation, getAIHint } from "./services/tutorService";
...
export default function App(){
 ...
 const visibleStudentQuestions=useMemo(()=> (Array.isArray(state.questions)?state.questions:[]).filter(q=>q.status!=="Draft" && (!q.educationLevel||q.educationLevel===session?.educationLevel) && (!q.grade||q.grade===session?.grade)),[state.questions,session?.educationLevel,session?.grade]);
 ...
 const handleAnswer=async(q,aIndex,modeName="practice",durationSeconds=0,meta={})=>{
   ...
   if(modeName!=="exam" && d.correct){
     // The teacher-authored question is only the starting point. Every
     // subsequent question in adaptive practice is generated by AI.
     generatedQuestion=await generateNextPracticeQuestion({
       currentQuestion:q,
       studentModel:next,
   ...
   const generationError=d.correct && generatedQuestion && !generatedQuestion.ai
     ? (generatedQuestion.error || generatedQuestion.code || "AI belum berhasil membuat soal berikutnya.")
     : null;
   return {...d,recommendation:null,generatedQuestion:d.correct && generatedQuestion?.ai ? generatedQuestion : null,generationError};
 };
 const handleGenerateNextQuestion=async(q,meta={})=>{
   const hints=Number(meta?.hintsUsed||0);
   const generated=await generateNextPracticeQuestion({
     currentQuestion:q,
     studentModel:state.studentModel,
     hintsUsed:hints,
     targetLevel:Math.max(1,Math.min(5,Number(q?.level||q?.difficulty||1)+(hints===0?1:0))),
     previousOutcome:{correct:Boolean(meta?.correct),selectedAnswer:meta?.selectedAnswer||null,correctAnswer:meta?.correctAnswer||null,misconceptionTag:meta?.misconceptionTag||null},
     context:{conceptId:q?.conceptId,conceptName:availableConcepts.find(c=>c.id===q?.conceptId)?.name||q?.conceptId,educationLevel:session?.educationLevel,grade:session?.grade,school:session?.school}
   });
   return generated;
 };
 ...
 const handlePracticeHint=async({question,hintIndex,conceptId})=>{
   const context={conceptId,conceptName:availableConcepts.find(c=>c.id===conceptId)?.name||conceptId,educationLevel:session?.educationLevel,grade:session?.grade,school:session?.school,question:question?.question,equation:question?.equation};
   const result=await getAIHint({question,context,hintLevel:hintIndex});
   if(session?.uid)await recordLearningEvent({uid:session.uid,name:session.name,teacherUid:session.classTeacherUid||null,classId:session.classId||null,type:"practice_hint",mode:"practice",conceptId,questionId:question?.id||null,payload:{hintIndex,hint:result?.hint||"",ai:Boolean(result?.ai)},educationLevel:session.educationLevel,grade:session.grade,rombel:session.rombel,school:session.school});
   return result;
 };
...
function TeacherGrades({students,session,questions,attempts=[],events=[],examResults=[],onRefresh,teacherClasses=[]}){
 ...
 const avg=(key)=>visible.length?Math.round(visible.reduce((sum,s)=>sum+Number(s[key]||0),0)/visible.length):0; const practiceMap=new Map();attempts.filter(a=>(a.mode||"practice")==="practice").forEach(a=>{const x=practiceMap.get(a.uid)||{n:0,c:0};x.n++;if(a.correct)x.c++;practiceMap.set(a.uid,x)}); const attemptCount=new Map();attempts.forEach(a=>attemptCount.set(a.uid,(attemptCount.get(a.uid)||0)+1)); const eventCount=new Map();events.forEach(e=>eventCount.set(e.uid,(eventCount.get(e.uid)||0)+1)); const examMap=new Map(examResults.map(r=>[r.uid,r])); const practiceScores=visible.map(s=>{const x=practiceMap.get(s.uid)||{n:0,c:0};return x.n?Math.round(x.c/x.n*100):0}); const avgPractice=practiceScores.length?Math.round(practiceScores.reduce((a,b)=>a+b,0)/practiceScores.length):0;
 return <div><div className="page-kicker">Student Assessment</div><h1 className="page-title">Nilai Siswa</h1><p className="page-desc">Filter sekolah, kelas, dan rombel. Siswa mengikuti kelas yang dikelola Guru.</p><div className="card" style={{marginBottom:18}}><div className="section-head"><div><h2>Kontrol Kelas</h2><span>Pilih ruang kelas yang sedang dilihat.</span></div><button className="btn" onClick={onRefresh}>↻ Perbarui</button></div><div className="register-grid"><div><label className="label">Sekolah</label><select value={school} onChange={e=>{setSchool(e.target.value);setGrade("");setRombel("")}}><option value="">Semua sekolah</option>{schools.map(v=><option key={v}>{v}</option>)}</select></div><div><label className="label">Kelas</label><select value={grade} onChange={e=>{setGrade(e.target.value);setRombel("")}}><option value="">Semua kelas</option>{grades.map(v=><option key={v}>{v}</option>)}</select></div><div><label className="label">Rombel</label><select value={rombel} onChange={e=>setRombel(e.target.value)}><option value="">Semua rombel</option>{rombels.map(v=><option key={v}>{v}</option>)}</select></div></div></div><div className="stats-row"><div className="ac-stat"><div className="stat-icon purple">▣</div><div><span>Rata-rata Mastery</span><strong>{avg("mastery")}%</strong></div></div><div className="ac-stat"><div className="stat-icon blue">✓</div><div><span>Siswa</span><strong>{visible.length}</strong></div></div><div className="ac-stat"><div className="stat-icon green">★</div><div><span>Rata-rata Latihan</span><strong>{avgPractice}%</strong></div></div><div className="ac-stat"><div className="stat-icon blue">✓</div><div><span>Rata-rata Ujian</span><strong>{avg("exam")}</strong></div></div><div className="ac-stat"><div className="stat-icon orange">✎</div><div><span>Bank Soal</span><strong>{questions.length}</strong></div></div></div><div className="card"><h2>Rekap Nilai</h2>{visible.length?<table className="table"><thead><tr><th>Siswa</th><th>Sekolah</th><th>Kelas</th><th>Attempt Latihan</th><th>Nilai Latihan</th><th>Ujian</th><th>Mastery</th><th>Aktivitas</th><th>XP</th></tr></thead><tbody>{visible.map(s=><tr key={s.uid||s.email}><td><strong>{s.name}</strong></td><td>{s.school||"-"}</td><td>{s.grade||"-"} {s.rombel||""}</td><td>{practiceMap.get(s.uid)?.n||0}</td><td>{practiceMap.get(s.uid)?.n?Math.round(practiceMap.get(s.uid).c/practiceMap.get(s.uid).n*100):0}%</td><td>{examMap.get(s.uid)?.score ?? s.exam ?? 0}</td><td>{s.mastery||0}%</td><td>{eventCount.get(s.uid)||0}</td><td>{s.xp||0}</td></tr>)}</tbody></table>:<div className="empty-state"><strong>{teacherClasses.length?"Belum ada siswa pada kelas ini.":"Belum ada kelas."}</strong><span>Daftar akan terisi otomatis setelah siswa terhubung ke kelas.</span></div>}</div></div>
}
