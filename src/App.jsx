...
export default function App(){const [session,setSession]=useState(getSession());const [state,setState]=useState(()=>loadState(fallback));const [tutorMessages,setTutorMessages]=useState([{role:"assistant",text:"Halo! Saya AI Tutor E-Comic. Kamu bisa bertanya tentang panel atau konsep yang sedang dipelajari."}]);const [drawerOpen,setDrawerOpen]=useState(false);
 ...
 useEffect(()=>{let alive=true;let unsubscribe=()=>{};(async()=>{const cloud=await loadCloudComics();if(!alive)return;if(cloud!==null){setState(s=>({...s,comics:mergeComicCollections(s.comics,cloud)}));unsubscribe=await subscribeCloudComics(items=>{if(alive)setState(s=>({...s,comics:mergeComicCollections(s.comics,items)}));});}})();return()=>{alive=false;unsubscribe();};},[session?.role]);
 ...
 const handleSaveComic=async u=>{const updated={...u,updatedAt:new Date().toISOString().slice(0,10)};setState(s=>({...s,comics:updateComic(s.comics,updated)}));const ok=await saveCloudComic(updated);return ok;};
