...
export default function App(){
 ...
 const handleAnswer=async(q,aIndex,modeName="practice",durationSeconds=0,hintsUsed=0)=>{
   ...
   if(modeName!=="exam"){
     recommendation=await recommendNextQuestion({studentModel:next,questions:visibleStudentQuestions,recentAttempts:[{questionId:q.id,conceptId:q.conceptId,correct:d.correct,hintsUsed:Number(hintsUsed)||0,level:Number(q.level??q.difficulty??1)}]});
 ...
 const handlePracticeHint=async({question,hintIndex,conceptId})=>{if(!session?.uid)return;await recordLearningEvent({uid:session.uid,name:session.name,teacherUid:session.classTeacherUid||null,classId:session.classId||null,type:"practice_hint",mode:"practice",conceptId,questionId:question?.id||null,payload:{hintIndex},educationLevel:session.educationLevel,grade:session.grade,rombel:session.rombel,school:session.school});};
