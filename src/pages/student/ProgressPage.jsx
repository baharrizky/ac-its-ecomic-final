...
export default function ProgressPage({studentModel,concepts=[]}){
 ...
 <div className="card"><h2>Mastery Konsep</h2><div className="mastery-list">{Object.entries(studentModel.concepts||{}).sort((a,b)=>Number(b[1]?.mastery||0)-Number(a[1]?.mastery||0)).map(([id,p])=><div className="mastery-row" key={id}><div className="mastery-name"><strong>{id}</strong><span>{concepts.find(c=>c.id===id)?.name || id}</span></div><div className="progress"><span style={{width:`${p.mastery*100}%`}}/></div><b>{Math.round(p.mastery*100)}%</b></div>)}</div></div>
 </div>
}
