...
const renderMathText=v=>{
  const text=String(v||"");
  if(!text.trim()) return null;
  const parts=text.split(/([A-Za-z0-9]+\^[A-Za-z0-9]+|[A-Za-z0-9]+\/[A-Za-z0-9]+)/g);
  return parts.map((part,i)=>{
    const raw=part||"";
    const isPower=/^[A-Za-z0-9]+\^[A-Za-z0-9]+$/.test(raw);
    const isFraction=/^[A-Za-z0-9]+\/[A-Za-z0-9]+$/.test(raw);
    if(!isPower&&!isFraction) return <React.Fragment key={i}>{raw}</React.Fragment>;
    const latex=isFraction?`\\frac{${raw.split("/")[0]}}{${raw.split("/")[1]}}`:raw;
    try{return <span key={i} dangerouslySetInnerHTML={{__html:katex.renderToString(latex,{displayMode:false,throwOnError:false})}}/>}catch{return <React.Fragment key={i}>{raw}</React.Fragment>}
  });
};
