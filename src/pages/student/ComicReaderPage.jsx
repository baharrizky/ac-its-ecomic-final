...
export default function ComicReaderPage({comic,studentModel,questions=[],navigate,onPanelViewed,onAnswer,onAIExplain,onTutorSend,tutorMessages=[],readerContext,session}){
 ...
 async function askAI(){if(!panel?.imageUrl)return;setTutorBusy(true);try{await onTutorSend?.(`Apa yang kamu lihat dan pahami dari gambar panel ini? Jelaskan bagian penting yang benar-benar terlihat.`,{...buildTutorContext(),_panelVision:true})}finally{setTutorBusy(false)}}
 ...
 async function sendTutor(e){e.preventDefault();const msg=tutorInput.trim();if(!msg||tutorBusy||!panel?.imageUrl)return;setTutorInput("");setTutorBusy(true);try{await onTutorSend?.(msg,{...buildTutorContext(),_panelVision:true})}finally{setTutorBusy(false)}}
 const content=<div className={`reader-page ${focusMode?"reader-page-focus":""}`}>
  ...
  <div className="reader-shell">
   ...
   <aside className="reader-side-card"><div className="reader-side-tabs">{tabs.map(({id,label,icon:Icon})=><button key={id} className={infoTab===id?"active":""} onClick={()=>setInfoTab(id)}><Icon size={15}/><span>{label}</span></button>)}</div><div className="reader-side-body">
    {infoTab==="tutor"&&<div className="reader-info-panel ai reader-tutor-panel"><div className="reader-info-icon"><MessageCircle size={18}/></div><h3>AI Tutor · Panel ini</h3><p className="reader-tutor-intro">Tanya langsung tentang gambar, cerita, atau konsep pada panel. AI Tutor membaca gambar panel yang sedang kamu lihat.</p><div className="reader-inline-chat" ref={tutorMessagesRef}>{tutorMessages.slice(-12).map((m,i)=><div key={i} className={`reader-chat-bubble ${m.role}`}><AIResponse text={m.text}/></div>)}{tutorBusy&&<div className="reader-chat-bubble assistant"><span className="reader-chat-typing">AI sedang berpikir…</span></div>}</div><form className="reader-chat-form" onSubmit={sendTutor}><input value={tutorInput} onChange={e=>setTutorInput(e.target.value)} placeholder="Tanya tentang panel ini…" disabled={tutorBusy}/><button className="btn-primary" disabled={tutorBusy||!tutorInput.trim()}>{tutorBusy?"…":"Kirim"}</button></form><button className="reader-chat-suggest" type="button" onClick={askAI} disabled={tutorBusy}>↗ Minta AI menjelaskan panel ini</button></div>}
