...
export default function ComicReaderPage({comic,studentModel,questions=[],navigate,onPanelViewed,onAnswer,onAIExplain,onTutorSend,tutorMessages=[],readerContext,session}){
 ...
 async function askAI(){if(!onTutorSend)return;setTutorBusy(true);try{await onTutorSend(`Apa yang kamu lihat dan pahami dari panel ini? Jelaskan bagian pentingnya terlebih dahulu.`,buildTutorContext())}finally{setTutorBusy(false)}}
 ...
 async function sendTutor(e){e.preventDefault();const msg=tutorInput.trim();if(!msg||tutorBusy||!onTutorSend)return;setTutorInput("");setTutorBusy(true);try{await onTutorSend(msg,buildTutorContext())}finally{setTutorBusy(false)}}
