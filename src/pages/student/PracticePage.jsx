...
export default function PracticePage({ questions = [], studentModel = {}, concepts = [], onAnswer, onAIExplain, onHint }) {
  ...
  async function useHint(){
    ...
    setHintLevel(next);
    setHintsUsed(v=>v+1);
    await onHint?.({question:q,hintIndex:next,conceptId:q.conceptId});
  }
  function retry(){
    setSelected(null);
    setDiagnosis(null);
  ...
  async function askAI(){
    ...
    try {
      const r=await onAIExplain({
        message:`Bantu siswa memahami kesalahan pada soal ini tanpa langsung memberikan jawaban akhir. Jelaskan konsep dan langkah berpikir yang perlu diperhatikan.`,
