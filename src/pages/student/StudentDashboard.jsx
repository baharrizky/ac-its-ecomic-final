...
export default function StudentDashboard({state,navigate,session,concepts=[]}) {
  const learnedConcepts = Object.entries(state.studentModel.concepts || {}).filter(([,p]) => Number(p?.attempts || 0) > 0);
      ...
      <div className="dashboard-grid">
        ...
        <aside>
          ...
          <div className="recommend-card">
            ...
            <span>Konsep yang sudah dipelajari dan perlu diperkuat</span>
            <strong>{weakest ? `${weakest[0]} · ${concepts.find(c=>c.id===weakest[0])?.name || weakest[0]}` : "Belum ada konsep yang sudah dilalui"}</strong>
            <div className="progress-label"><span>Mastery</span><b>{weakest ? `${Math.round((weakest[1]?.mastery||0)*100)}%` : "-"}</b></div>
            <div className="progress"><span style={{width:`${(weakest?.[1]?.mastery||0)*100}%`}}/></div>
            <button className="primary-btn small full" disabled={!weakest} onClick={()=>navigate("practice")}>{weakest ? "Mulai Latihan" : "Mulai dari E-Comic"}</button>
          </div>
        </aside>
      </div>
    </div>
