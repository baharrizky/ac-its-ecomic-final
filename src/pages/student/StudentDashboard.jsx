...
export default function StudentDashboard({state,navigate,session,concepts=[]}) {
  const conceptProfiles = state.studentModel?.concepts || {};
  const attemptedConcepts = Object.entries(conceptProfiles).filter(([,p])=>Number(p?.attempts||0)>0);
  const weakest = attemptedConcepts.sort((a,b)=>Number(a[1]?.mastery||0)-Number(b[1]?.mastery||0))[0];
      ...
      <div className="dashboard-grid">
        ...
        <aside>
          ...
          <div className="recommend-card">
            ...
            <strong>{weakest ? `${weakest[0]} · ${concepts.find(c=>c.id===weakest[0])?.name || weakest[0]}` : "Belum ada konsep yang sudah dipelajari"}</strong>
