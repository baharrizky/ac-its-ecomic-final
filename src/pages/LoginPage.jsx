...
import { GraduationCap, ShieldCheck, Users, ArrowLeft, LogIn, UserPlus } from "lucide-react";
import { demoAccounts, login, resetPassword } from "../services/authService";
  ...
  return <div className="login-page">
    ...
    <div className="login-shell">
      ...
      <div className="login-card">
        {!role ? <><div className="login-card-head"><h2>Masuk ke AC-ITS</h2><p>Pilih jenis akun untuk melanjutkan.</p></div><button className="role-card" onClick={()=>choose("student")}><div className="role-icon student"><Users/></div><div><strong>Login Siswa</strong><span>Masuk ke ruang belajar, E-Comic, AI Tutor, dan latihan adaptif.</span></div><b>→</b></button><button className="role-card" onClick={()=>choose("teacher")}><div className="role-icon teacher"><GraduationCap/></div><div><strong>Login Guru</strong><span>Kelola E-Comic, materi, soal, analitik, dan kelas.</span></div><b>→</b></button><button className="role-card" onClick={()=>choose("admin")}><div className="role-icon teacher"><ShieldCheck/></div><div><strong>Login Admin</strong><span>Kelola akses Guru, kode registrasi, dan kontrol platform.</span></div><b>→</b></button></> : <form onSubmit={submit}><button type="button" className="back-login" onClick={()=>setRole(null)}><ArrowLeft size={15}/> Kembali</button><div className="login-card-head"><h2>Login {role === "teacher" ? "Guru" : role === "admin" ? "Admin" : "Siswa"}</h2><p>{role === "teacher" ? "Teacher Content & Analytics Space" : role === "admin" ? "Platform Administration Space" : "Student Adaptive Learning Space"}</p></div><label className="label">Email</label><input type="email" value={email} onChange={e=>setEmail(e.target.value)}/><label className="label">Password</label><input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Masukkan password"/>{error&&<div className="login-error">{error}</div>}{resetMessage&&<div className="success-note">{resetMessage}</div>}<button type="button" className="text-btn" style={{justifyContent:"flex-end",margin:"-2px 0 8px"}} disabled={resetBusy} onClick={async()=>{setResetMessage("");setError("");setResetBusy(true);const r=await resetPassword(email);setResetBusy(false);if(r.ok)setResetMessage(r.message);else setError(r.message)}}>{resetBusy?"Mengirim link…":"Lupa password?"}</button><button className="btn-primary login-submit"><LogIn size={16}/> Masuk</button><button type="button" className="register-link" onClick={onRegisterClick}><UserPlus size={15}/> Belum punya akun? Daftar</button></form>}
      </div>
    </div>
