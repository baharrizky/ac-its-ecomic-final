...
export default function QuestionBankPage({ questions=[], comics=[], teacherClasses=[], session, onSave, onDelete }) {
  ...
  function openNew(){const c=teacherClasses.find(x=>x.school===session?.school)||teacherClasses[0];setEditing({...emptyQuestion,id:`q-${Date.now()}`,school:c?.school||session?.school||"",educationLevel:c?.educationLevel||"SMA",grade:c?.grade||"X"});}
  function save(){
  ...
  return <div>
    ...
    {editing&&<div className="modal-wrap"><div className="modal" style={{maxWidth:820}}><div className="modal-head"><div><div className="page-kicker">Question Authoring</div><h2>{editing.id?.startsWith("q-")?"Tambah Soal":"Edit Soal"}</h2></div><button className="close" onClick={()=>setEditing(null)}>×</button></div>
      ...
      <div className="register-grid"><div className="field"><label className="label">Status</label><select value={editing.status||"Published"} onChange={e=>setEditing({...editing,status:e.target.value})}><option>Draft</option><option>Published</option></select></div><div className="field"><label className="label">Tag miskonsepsi</label><input value={(editing.misconceptionTags||[]).join(", ")} onChange={e=>setEditing({...editing,misconceptionTags:e.target.value.split(",").map(x=>x.trim()).filter(Boolean)})}/></div></div><div className="field"><label className="label">Pembahasan</label><textarea rows="3" value={editing.explanation} onChange={e=>setEditing({...editing,explanation:e.target.value})}/></div>
