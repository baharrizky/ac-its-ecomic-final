...
const emptyQuestion = { id:"", question:"", equation:"", options:["","","",""], answer:0, explanation:"", conceptId:"", level:1, educationLevel:"SMA", grade:"X", school:"", status:"Published", assessmentType:"practice", comicId:"", episodeId:"", panelId:"", hints:["","",""] };
...
export default function QuestionBankPage({ questions=[], comics=[], teacherClasses=[], session, onSave, onDelete }) {
  ...
  function save(){
    ...
    onSave({...editing,question:editing.question.trim(),options:editing.options.map(o=>o.trim()),difficulty:Number(editing.level||1),assessmentType:editing.assessmentType||"practice",misconceptionTags:(misconceptionInput.split(",").map(x=>x.trim()).filter(Boolean).length?misconceptionInput.split(",").map(x=>x.trim()).filter(Boolean):["UNCLASSIFIED"]),assignedClassIds:[]});
    setEditing(null);
  }
  ...
  return <div>
    ...
    {editing&&<div className="modal-wrap"><div className="modal" style={{maxWidth:820}}><div className="modal-head"><div><div className="page-kicker">Question Authoring</div><h2>{editing.id?.startsWith("q-")?"Tambah Soal":"Edit Soal"}</h2></div><button className="close" onClick={()=>{setEditing(null);setMisconceptionInput("");}}>×</button></div>
      <div className="register-grid"><div className="field"><label className="label">Jenis soal</label><select value={editing.assessmentType||"practice"} onChange={e=>setEditing({...editing,assessmentType:e.target.value})}><option value="practice">Latihan Adaptif</option><option value="exam">Ujian</option></select></div><div className="field"><label className="label">Jenjang</label><select value={editing.educationLevel} onChange={e=>setEditing({...editing,educationLevel:e.target.value,grade:e.target.value==="SMP"?"VII":"X",comicId:"",episodeId:"",panelId:""})}><option>SMP</option><option>SMA</option></select></div><div className="field"><label className="label">Kelas</label><select value={editing.grade} onChange={e=>setEditing({...editing,grade:e.target.value})}>{(editing.educationLevel==="SMP"?["VII","VIII","IX"]:["X","XI","XII"]).map(g=><option key={g}>{g}</option>)}</select></div></div>
