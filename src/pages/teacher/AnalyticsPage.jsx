...
export default function AnalyticsPage({ students = [], models = [], attempts = [], events = [], examResults = [], reflections = [], teacherClasses = [], onRefresh }) {
  ...
  const safeStudents=Array.isArray(students)?students:[]; const safeModels=Array.isArray(models)?models:[]; const safeAttempts=Array.isArray(attempts)?attempts:[]; const safeEvents=Array.isArray(events)?events:[]; const safeExamResults=Array.isArray(examResults)?examResults:[];
  ...
  const filteredStudents=safeStudents.filter(s=>selectedIds.has(s.classId) || ((!s.classId)&&(!school||s.school===school)&&(!grade||s.grade===grade)&&(!rombel||String(s.rombel)===String(rombel))));
  const rows=filteredStudents.map(s=>({...s,model:byUid.get(s.uid)||{}}));
  const scopedAttempts=safeAttempts.filter(a=>!rombel && !grade && !school ? true : selectedIds.has(a.classId) || ((!a.classId)&&(!school||a.school===school)&&(!grade||a.grade===grade)&&(!rombel||String(a.rombel)===String(rombel))));
  ...
  const scopedEvents=safeEvents.filter(e=>scopedStudentIds.has(e.uid));
  ...
  const evaluationEvents=scopedAttempts.length + safeExamResults.filter(r=>scopedStudentIds.has(r.uid)).length;
  ...
  const topConcepts=conceptRows.slice().sort((a,b)=>b.value-a.value).slice(0,5);
  const priorityConcepts=conceptRows.slice().sort((a,b)=>a.value-b.value).slice(0,5);
