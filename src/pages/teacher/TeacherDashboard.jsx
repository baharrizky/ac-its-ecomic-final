...
export default function TeacherDashboard({state,navigate,students=[],models=[],attempts=[],events=[],session,onSeedPack,onAIRecommend}){
 ...
 return <div>
   <div className="page-kicker">Teacher Workspace</div><h1 className="page-title">Dashboard Guru</h1><p className="page-desc">Kelola konten E-Comic, pantau student model, dan gunakan AI untuk rekomendasi pembelajaran individual.</p>
   ...
   <div className="card" style={{marginTop:18}}>
    <div className="section-head"><div><h2>AI Teaching Assistant</h2><span>Rekomendasi individual berdasarkan student model, attempt, miskonsepsi, dan aktivitas siswa.</span></div><Sparkles size={20}/></div>
    ...
    {result&&<div className="ai-feedback" style={{marginTop:14}}><strong>{result.ai===false?"Fallback berbasis data":"Rekomendasi AI"}</strong><p>{result.summary}</p>{result.priorityConcepts?.length>0&&<div style={{marginTop:8}}><b>Prioritas konsep:</b> {result.priorityConcepts.join(", ")}</div>}{result.recommendations?.length>0&&<ul style={{margin:"8px 0 0 18px"}}>{result.recommendations.map((x,i)=><li key={i}>{x}</li>)}</ul>}{result.nextActivity&&<div style={{marginTop:8}}><b>Langkah berikutnya:</b> {result.nextActivity}</div>}{result.teacherNote&&<div style={{marginTop:8}}><b>Catatan guru:</b> {result.teacherNote}</div>}</div>}
   </div>
