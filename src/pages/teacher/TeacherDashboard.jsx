...
export default function TeacherDashboard({state,navigate,students=[],models=[],attempts=[],events=[],session,teacherClasses=[],onRefresh,onSeedPack,onAIRecommend}){
 const [selectedUid,setSelectedUid]=useState("");
 const [result,setResult]=useState(null);const [loading,setLoading]=useState(false);
 const scopedStudents=students.filter(s=>!session?.uid||s.classTeacherUid===session.uid||teacherClasses.some(c=>c.id===s.classId));
 useEffect(()=>{ if(onRefresh) onRefresh(); },[session?.uid]);
 ...
 return <div>
   ...
   <div className="stats-row"><div style={{gridColumn:"1/-1",display:"flex",justifyContent:"flex-end",marginBottom:-4}}><button className="btn" onClick={onRefresh}>↻ Perbarui data kelas & siswa</button></div>
