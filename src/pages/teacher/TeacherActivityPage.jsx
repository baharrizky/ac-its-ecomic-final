...
export default function TeacherActivityPage({students=[],events=[],onRefresh}){
  ...
  return <div>
    ...
    <div className="card" style={{marginBottom:16}}><div className="register-grid"><div><label className="label">Tanggal</label><input type="date" value={date} onChange={e=>setDate(e.target.value)}/></div><div><label className="label">Siswa</label><select value={studentUid} onChange={e=>setStudentUid(e.target.value)}><option value="">Semua siswa</option>{students.map(s=><option key={s.uid} value={s.uid}>{s.name} · {classLabel(s.grade, s.rombel)}</option>)}</select></div></div></div>
    ...
    <div className="card">
      ...
      {studentRows.length?<div style={{overflowX:"auto"}}><table className="table"><thead><tr><th>Siswa</th><th>Check-in</th><th>Akses</th><th>Baca Comic</th><th>Kuis</th><th>Latihan</th><th>Tutor</th><th>Aktivitas terakhir</th></tr></thead><tbody>{studentRows.map(r=><tr key={r.uid}><td><strong>{r.name}</strong><div className="subtle">{r.school||"-"} · {classLabel(r.grade, r.rombel)}</div></td><td>{r.checkins}</td><td>{fmt(r.access)}</td><td>{fmt(r.comic)}</td><td>{fmt(r.quiz)}</td><td>{fmt(r.practice)}</td><td>{fmt(r.tutor)}</td><td>{r.last?new Date(r.last).toLocaleTimeString("id-ID",{hour:"2-digit",minute:"2-digit"}):"-"}</td></tr>)}</tbody></table></div>:<div className="empty-state"><strong>Belum ada siswa pada filter ini.</strong><span>Data aktivitas akan muncul setelah siswa mulai menggunakan platform.</span></div>}
    </div>
