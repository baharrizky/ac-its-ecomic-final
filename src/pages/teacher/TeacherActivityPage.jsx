...
export default function TeacherActivityPage({students=[],events=[],teacherClasses=[],onRefresh}){
  const [date,setDate]=useState(today());
  const [studentUid,setStudentUid]=useState("");
  const [classId,setClassId]=useState("");
  const classStudents=useMemo(()=>classId?students.filter(s=>s.classId===classId):students,[students,classId]);
  const visibleEvents=useMemo(()=>events.filter(e=>!date||dayOf(e.createdAt||e.startedAt||e.endedAt)===date).filter(e=>!classId||e.classId===classId).filter(e=>!studentUid||e.uid===studentUid),[events,date,studentUid,classId]);
  const studentMap=new Map(students.map(s=>[s.uid,s]));
  const scopedStudents=studentUid?classStudents.filter(s=>s.uid===studentUid):classStudents;
  ...
  return <div>
    ...
    <div className="card" style={{marginBottom:16}}><div className="register-grid"><div><label className="label">Tanggal</label><input type="date" value={date} onChange={e=>setDate(e.target.value)}/></div><div><label className="label">Sekolah / Kelas</label><select value={classId} onChange={e=>{setClassId(e.target.value);setStudentUid("")}}><option value="">Semua kelas</option>{teacherClasses.map(c=><option key={c.id} value={c.id}>{c.school} · {c.grade} {c.rombel}</option>)}</select></div><div><label className="label">Siswa</label><select value={studentUid} onChange={e=>setStudentUid(e.target.value)}><option value="">Semua siswa</option>{classStudents.map(s=><option key={s.uid} value={s.uid}>{s.name} · {s.grade||"-"} {s.rombel||""}</option>)}</select></div></div></div>
