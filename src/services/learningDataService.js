...
const LOCAL_PREFIX = "ac-its-ecomic-data-v1:";
...
export async function getStudentModel(uid, fallback) {
  ...
  if (await ready()) {
    try {
      const snap = await getDoc(doc(db, "studentModels", uid));
      if (snap.exists()) return { ...fallback, ...snap.data() };
      await setDoc(doc(db, "studentModels", uid), { ...fallback, uid, updatedAt: new Date().toISOString() }, { merge: true });
...
export async function saveStudentModel(uid, model) {
  ...
  if (uid && await ready()) {
    try { await setDoc(doc(db, "studentModels", uid), payload, { merge: true }); return true; }
...
export async function subscribeStudentModel(uid, onChange) {
  ...
  return onSnapshot(doc(db, "studentModels", uid), snap => { if (snap.exists()) onChange(snap.data()); }, error => console.warn("studentModel subscription failed", error));
}
...
export async function recordLearningEvent(event = {}) {
  ...
  localUpsert("learningEvents", id, { ...item, id });
  if (await ready()) {
    try { await setDoc(doc(db, "learningEvents", id), { ...item, id }, { merge: true }); return true; }
...
export async function recordAttempt(attempt = {}) {
  ...
  if (await ready()) {
    try { await setDoc(doc(db, "attempts", item.id), item, { merge: true }); return true; }
...
export async function listStudentModels() {
  if (await ready()) {
    try {
      const snap = await getDocs(collection(db, "studentModels"));
      return snap.docs.map(d => ({ uid: d.id, ...d.data() }));
    } catch (e) { console.warn("student models list failed", e); }
  }
  const rows = localRead("studentModels", []);
...
export async function saveReflection(reflection = {}) {
  ...
  localUpsert("reflections", item.id, item);
  if (await ready()) {
    try { await setDoc(doc(db, "reflections", item.id), item, { merge: true }); return true; }
...
export async function listReflections() {
  if (await ready()) {
    try { const snap = await getDocs(collection(db, "reflections")); return snap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a,b)=>String(b.createdAt||"").localeCompare(String(a.createdAt||""))); }
    catch (e) { console.warn("reflection list failed", e); }
  }
  return localRead("reflections", []);
}
...
export async function saveAttendance(item = {}) {
  ...
  localUpsert("attendance", record.id, record);
  if (await ready()) {
    try { await setDoc(doc(db, "attendance", record.id), record, { merge: true }); return true; }
...
export async function createClassAccessCode(data = {}) {
  ...
  localUpsert("classAccessCodes", id, item);
  if (await ready()) {
    try { await setDoc(doc(db, "classAccessCodes", id), item, { merge: true }); return item; }
...
export async function listClassAccessCodes(teacherUid) {
  if (await ready()) {
    try {
      const snap = await getDocs(collection(db, "classAccessCodes"));
      let rows = snap.docs.map(d => ({ id:d.id, ...d.data() }));
      if (teacherUid) rows = rows.filter(r => !r.teacherUid || r.teacherUid === teacherUid);
      return rows.sort((a,b)=>String(b.createdAt||"").localeCompare(String(a.createdAt||"")));
    } catch (e) { console.warn("class code list failed", e); }
  }
  let rows = localRead("classAccessCodes", []);
  if (teacherUid) rows = rows.filter(r => !r.teacherUid || r.teacherUid === teacherUid);
  return rows;
}

export async function deactivateClassAccessCode(id) {
  const rows = localRead("classAccessCodes", []);
  localWrite("classAccessCodes", rows.map(r => r.id === id ? { ...r, active:false } : r));
  if (await ready()) { try { await setDoc(doc(db, "classAccessCodes", id), { active:false }, { merge:true }); return true; } catch {} }
  return false;
}

export async function saveKnowledgeItem(item = {}) {
  const record = { ...item, id: item.id || `kb-${Date.now()}`, updatedAt:new Date().toISOString() };
  localUpsert("knowledgeBase", record.id, record);
  if (await ready()) { try { await setDoc(doc(db,"knowledgeBase",record.id),record,{merge:true}); return true; } catch(e){ console.warn("knowledge write failed",e); } }
  return false;
}

export async function deleteKnowledgeItem(id) {
  localWrite("knowledgeBase", localRead("knowledgeBase", []).filter(r=>r.id!==id));
  if(await ready()){try{await deleteDoc(doc(db,"knowledgeBase",id));return true;}catch{}}
  return false;
}

export async function listKnowledgeItems() {
  if(await ready()){try{const snap=await getDocs(collection(db,"knowledgeBase"));return snap.docs.map(d=>({id:d.id,...d.data()}));}catch(e){console.warn("knowledge list failed",e);}}
  return localRead("knowledgeBase",[]);
}
...
export async function saveExamResult(result = {}) {
  ...
  localUpsert("examResults", item.id, item);
  if(await ready()){try{await setDoc(doc(db,"examResults",item.id),item,{merge:true});return true;}catch(e){console.warn("exam result failed",e);}}
  return false;
}

export async function listExamResults() {
  if(await ready()){try{const snap=await getDocs(collection(db,"examResults"));return snap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>String(b.createdAt||"").localeCompare(String(a.createdAt||"")));}catch(e){console.warn("exam list failed",e);}}
  return localRead("examResults",[]);
}
