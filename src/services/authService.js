...
export function login(role, email, password){
  ...
  const session = { role:account.role, name:account.name, subtitle:account.subtitle, email:account.email, educationLevel:account.educationLevel || null, grade:account.grade || null, loggedAt:new Date().toISOString() };
